...
<div class="flex h-full min-h-0 flex-col gap-3 overflow-y-auto">
	<div class="flex items-center gap-5">
		<div class="shrink-0">
			...
			<div class="text-3xl font-semibold tracking-tight text-app-fg tabular-nums">
				{formatPercent(totalPct)}
			</div>
		</div>
		<div class="min-h-0 flex-1">
			<Chart values={total} max={100} height={72} color="#5b8cff" />
		</div>
	</div>

	<div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
		<div class="rounded-xl border border-app-border bg-app-card2 px-3 py-2">
			<div class="text-[10px] font-medium tracking-wide text-app-mute uppercase">Freq</div>
			<div class="mt-0.5 truncate text-sm font-semibold text-app-fg tabular-nums">
				{cpu.frequency}
			</div>
		</div>
		<div class="rounded-xl border border-app-border bg-app-card2 px-3 py-2">
	...
	<div class="min-h-0 flex-1">
		<div class="mb-2 text-[11px] font-medium text-app-dim">Cores ({cores.length})</div>
		<div class="grid grid-cols-1 gap-x-5 gap-y-1.5 sm:grid-cols-2 lg:grid-cols-3">
